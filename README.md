...
4. 設定ファイルの作成:
   ...
   - `exclude_account_ids`には、通常、自分のアカウント ID を入れます。これにより、自分宛てのメッセージは自動既読にされません。
   - `[toall]`は自動既読にされません。
   - 複数のアカウント ID を設定する場合は、カンマで区切って入力します。例：
     ```
     exclude_account_ids = ["123456", "789012", "345678"]
     ```
   - `exclude_room_ids`には、自動既読にしたくないチャットルームの ID を入れます。これも複数ある場合はカンマで区切ります。例：
     ```
     exclude_room_ids = [11111, 22222, 33333]
     ```
   ...
   注意: アカウント ID は `exclude_account_ids` ではダブルクォーテーション（"）で囲み、ルーム ID は `exclude_room_ids` では囲まないでください。
